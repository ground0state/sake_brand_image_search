...
criterion = nn.CrossEntropyLoss(reduction="mean").to(C.train_device)
# criterion = FocalLoss(logits=True)
loss_weight = 1
centerloss = CenterLoss(C.num_classes, C.embedding_dim).to(C.train_device)
...
for epoch in range(1, C.num_epochs+1):
    ...
    for step, (inputs, labels) in enumerate(train_loader, start=1):
        ...
        center_loss = centerloss(labels, embed)[0]
        loss_all = loss + loss_weight * center_loss
        ...
        total_center_loss += center_loss.item() * inputs.size(0)
        total_num_data += inputs.size(0)

        if step % C.log_interval == 0:
            running_loss = total_loss / total_num_data
            running_center_loss = total_center_loss / total_num_data
            logger.info(
            ...
            writer.add_scalar("Train/CenterLoss", running_center_loss,
                              (epoch-1) * num_steps + step)
