...
metric = losses.ArcFaceLoss(
    num_classes=C.num_classes,
    embedding_size=C.embedding_dim,
    scale=64,
    margin=0.5)
metric.to(C.train_device)
...
for epoch in range(1, C.num_epochs+1):
    ...
    for step, (inputs, labels) in enumerate(train_loader, start=1):
        ...
        metric_loss = metric(embed, labels)
        all_loss = loss + metric_loss
        ...
        metric_total_loss += metric_loss.item() * inputs.size(0)
        total_num_data += inputs.size(0)

        if step % C.log_interval == 0:
            running_loss = total_loss / total_num_data
            running_metric_loss = metric_total_loss / total_num_data
            logger.info(
            ...
            writer.add_scalar("Train/MetricLoss", running_metric_loss,
                              (epoch-1) * num_steps + step)
